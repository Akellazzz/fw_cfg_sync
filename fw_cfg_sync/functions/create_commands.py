...
def _active_only_commands(
    active_config: list, reserve_config: list, active_delta: list, reserve_delta: list
) -> list:
...
def _negate_reserve_only_commands(
    active_config: list, reserve_config: list, active_delta: list, reserve_delta: list
) -> list:
                ...
                if act_block_parse.ioscfg == res_block_parse.ioscfg:
                    ...
                    if block_intersection:
                        ...
                        if len(block_intersection) > 1:
                            intersection = list(
                                dict.fromkeys(block_intersection)
                            )  # Remove Duplicates
                            cmd_tmp_list = []
                            parent = block_intersection[0]  # parent
                            children = block_intersection[1::]
                            children = sorted(
                                children, key=lambda a: a.strip().startswith("no ")
                            )  # команды на удаление в конец списка
...
def create_commands(
    ...
    commands = _active_only_commands(
        active_config, reserve_config, active_delta, reserve_delta
    )
    no_commands = _negate_reserve_only_commands(
        active_config, reserve_config, active_delta, reserve_delta
    )
    atomic_changes = _intersection(
        active_config, reserve_config, active_delta, reserve_delta
    )
