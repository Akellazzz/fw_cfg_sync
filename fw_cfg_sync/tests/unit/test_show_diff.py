...
def test_without_empty_tr():
    ...
    file1 = os.path.join(pytest.tests_dir, "fw_configs_for_tests", "with_empty_tr1.txt")
    file2 = os.path.join(pytest.tests_dir, "fw_configs_for_tests", "with_empty_tr2.txt")
    ...
    assert "only2" not in uniq_in_file1
    assert "access-list test extended permit icmp any any" not in uniq_in_file1
    ...
    assert (
        "access-list test21 extended permit ip any any time-range empty_tr2"
        in uniq_in_file2
    )
    assert (
        "access-list test22 extended permit ip any any time-range not_empty_tr2"
        in uniq_in_file2
    )
    assert "access-list test23 extended permit icmp any any" in uniq_in_file2

    assert "absolute end 00:00 16 December 2025" in uniq_in_file2
